...
using ParallelListLayout = PrimitiveEngine.Unity.ReorderableListAttribute.ParallelListLayout;
using BackgroundColorDelegate = PrimitiveEngine.Unity.ReorderableListDrawer.BackgroundColorDelegate;
using System.Collections;

namespace PrimitiveEngine.Unity
{

    public class ReorderableListOfValues : ReorderableList
    {
        public Action<int> OnElementRemoved;
        ...
        public ReorderableListDrawer.BackgroundColorDelegate onBackgroundColor;

        public readonly SerializedProperty[] serializedProperties;

        public readonly ReorderableListAttribute.ParallelListLayout parallelListLayout;
            ...
            if (primaryProperty.isExpanded)
            {
                ...
                foreach (var array in serializedProperties)
                {
                    for (int i = 0; i < elementCount; ++i)
                    {
                        ...
                        {
                            case ReorderableListAttribute.ParallelListLayout.Rows:
                                m_ElementHeights[i] += elementHeight;
                                break;
                            case ReorderableListAttribute.ParallelListLayout.Columns:
                                m_ElementHeights[i] =
                                    Mathf.Max(
                                        m_ElementHeights[i],
                                        elementHeight);
